...
% LM 2026 modifed to work with dod gait project
...
function [warpedMNImesh, MNImesh, warping] = nft_warping_meshLM(subject_name, session_name, elec_file, nl, of, plotting, femmesh)
...
if plotting
  ...
  [MNImesh, electrodes, fiducials, index_kdm] = init_mesh(input_electrodes, plotting, eloc);  % LM added eloc for labels during plotting
...
%  chosenindices = ind;
  ...
  if plotting
    ...
    h.FaceAlpha = 0.55;
    plot3(Ptm(:,1), Ptm(:,2), Ptm(:,3), 'b.','MarkerSize',8)
    text(Ptm(:,1), Ptm(:,2), Ptm(:,3), {eloc(ind).labels}');
    axis([-100 100 -200 100 -100 150])
    view(165, 10)
    title('Head mesh WARPED')
    linkaxes([ax1 ax2],'xyz')
    
    saveas(gcf,'head_mesh_warped.fig');
  end
...
% save warped sensors and index
  ...
  f = [fsubj '_' num2str(fses)];   % LM - num2str(fses)
  warped_sensors = Ptm;
  ...
  % LM - save mesh for later pop_dipfit (POS=vertices, TRI1=faces, per headplot.m)
  smesh.POS  = Coord(:,2:4); 
  smesh.TRI1 = Elem(:,2:4); 
...
% LM - changed {labels X Y Z type} format + header to make it work with data
...
% LM - add eloc arg for plot labels
function [MNImesh, electrodes, fiducials, index_kdm] = init_mesh(input_electrodes, plotting, eloc)
...
%   rejected = setdiff(index,kdm);
  ...
  % LM - reject scalp-distance outliers via isoutlier quartile/1.5*IQR
  rejected = index(isoutlier(dm,'quartiles')); % find outlier using quartile method
  kdm =   index(~isoutlier(dm,'quartiles'));

  if plotting
    plot3(pos(:,1), pos(:,2), pos(:,3), 'b.')
    text(pos(:,1), pos(:,2), pos(:,3), {eloc.labels})
    plot3(pos(rejected,1),pos(rejected,2),pos(rejected,3),'ro','MarkerSize',9,'LineWidth',8)
    title('Standard Head Model with user localized channel locs')
    %saveas(gcf,'head_mesh_standard.fig');
  end
